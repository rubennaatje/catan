	...
	public void removeCardsIfMoreThan8() {
		try {
			...
			if(rs.getInt(1) > 7) {
				...
				int counter = 0;
				while(rs2.next()) {
					DatabaseManager.createStatement().executeUpdate("UPDATE `spelergrondstofkaart` SET `username`=NULL WHERE  `idspel`=" + getSpelId() + " AND `idgrondstofkaart`='" + rs2.getString(1) +"'");
					counter++;
